...
if __name__== '__main__':
    ...
    (x,y,w,h,area) = bin_segm.template_method(cv2.imread('E:/DZ5/NTZ8/data/1.jpg', cv2.IMREAD_GRAYSCALE))
    for i in range(len(area)):
            print([x[i], y[i], w[i],h[i],area[i]])

    print("Decorated Binary Image Processing")
    filt_bin = FilteredAnalysis(BinaryImage())
    (x, y, w, h, area, moments) = filt_bin.template_method(cv2.imread('E:/DZ5/NTZ8/data/1.jpg', cv2.IMREAD_GRAYSCALE))
    for i in range(len(area)):
        print([x[i], y[i], w[i], h[i], area[i], moments[i]])

    print("Color Image Processing")
    color_segm = ColorImage()
    (x, y, w, h, area) = color_segm.template_method(cv2.imread('E:/DZ5/NTZ8/data/1.jpg', cv2.IMREAD_COLOR))
    for i in range(len(area)):
        print([x[i], y[i], w[i], h[i], area[i]])
